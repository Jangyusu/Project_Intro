...
if(!isset($_POST['mode'])) {
    $query = "insert into project(title, url, date, upload, contents, state) values ('$title','$url','$date','$fileDir','$content','$state')";
} else {
    $num = $_POST['num'];
    if(!empty($fileName)){
    ...
    $query = "update project set title='$title', url='$url', date='$date', contents='$content', state='$state' where num='$num'";
}
    mq($query); //쿼리명령문 실행
